	...
	<div class="col-sm-4 col-md-4">
        ...
        <div class="datos-row">
        	@if(auth()->user()->provider=='google')
                ...
                <i class="icon-information fas fa-question-circle fa-lg"><span class="tooltip" title="Este es el provedor por el cual te registraste.">aa</span></i>
        	@endif
        ...
        <div class="datos-row">
        	...
        	<h6 class="datos-perfil">Te uniste {{auth()->user()->created_at->diffForHumans()}}</h6>
        </div>
        <div class="datos-row">
        ...
        <div class="datos-row">
        	...
        	<h6 class="datos-perfil">Modificado {{auth()->user()->updated_at->diffForHumans()}}</h6>
        </div>
        <div class="datos-row">
        	<i class="icon-perfil fas fa-user fa-lg"></i>
        	<h6 class="datos-perfil">{{$user_types[auth()->user()->user_type]}}</h6>
            <i class="icon-information fas fa-question-circle fa-lg"><span class="tooltip" title="{{$user_types_message[auth()->user()->user_type]}}">aa</span></i>

        </div>
        <div class="datos-row">
        	<i class="icon-perfil fas fa-info fa-lg"></i>
        	<h6 class="datos-perfil">Cuenta {{$user_status[auth()->user()->user_state]}}</h6>
            <i class="icon-information fas fa-question-circle fa-lg"><span class="tooltip" title="{{$user_message[auth()->user()->user_state]}}">aa</span></i>
	       ...
	       @if(auth()->user()->user_state==0)	
               ...
               <form method="post" action="{{ route('user_ubication') }}">
                ...
                @if(auth()->user()->user_state>=2)
                    ...
                    <input type="number" name="carnet" id="ci" class="input-line-yellow form-control">
                    @error('carnet')
                        <small><strong>{{ $message }}</strong></small>
                    @enderror
                    <label for="tel" class="text-label-left text-theme">Añade un teléfono celular</label>
                    <input type="number" name="telefono" id="tel" class="input-line-yellow form-control">
                    @error('telefono')
                        <small><strong>{{ $message }}</strong></small>
                    @enderror
